//==========================================================================
// ジオメトリーシェーダーのマネージャー [GeometryShaderManager.cpp]
// author : MasyoLab
//==========================================================================
...
namespace common
{
	GeometryShaderManager::GeometryShaderManager(DirectXDevice device) :
		m_device(device)
	{
	}

	GeometryShaderManager::~GeometryShaderManager()
	{
	}

	GeometryShader GeometryShaderManager::CreateGeometryShader(const std::string& cso_file_path)
	{
		// シェーダーが生成済みかをチェック
		...
		auto hr = m_device->CreateGeometryShader(load.GetData(), load.GetSize(), nullptr, &pShader);
		assert(SUCCEEDED(hr));
		if (FAILED(hr))
			return nullptr;
	...
	GeometryShader GeometryShaderManager::CompileGeometryShader(const std::string& filename, const std::string& entrypath, bool erorr)
	{
		// シェーダーが生成済みかをチェック
		...
		hr = m_device->CreateGeometryShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &pShader);
		assert(SUCCEEDED(hr));
